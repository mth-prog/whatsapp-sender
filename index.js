...
async function bot(mensagem = "",  Telefone= "", anexo= "") {

    const browser = await puppeteer.launch({headless: false});
    const page = await browser.newPage();
    this.page = page;
    
    ArrayTelefone = Telefone.split(",");
    ArrayAnexo = anexo.split(",")
    ...
    for(var i = 0; i < ArrayTelefone.length; ++i) {
      
      await page.goto(`https://web.whatsapp.com/send?phone=${ArrayTelefone[i]}`, {timeout: 360000, waitUntil: 'load'});
